...
/**
 * @title RhythmRushGem
 * @dev ERC721 NFT contract for RhythmRush play-to-earn game
 * Compatible with Celo network (supports CELO and cUSD payments)
 * NFTs are referred to as "GEMS" in the game
 */
contract RhythmRushGem is ERC721URIStorage, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Token price: 34 RUSH tokens per Gem (adjust based on your token decimals)
    uint256 public constant PRICE_PER_GEM = 34 * 10**18; // 34 RUSH tokens with 18 decimals
    ...
    // Payment token address (RhythmRushToken - RUSH, or cUSD)
     ...
     * @param _paymentToken Address of the payment token (cUSD or CELO)
    ...
    function claim(
        ...
        // Ignore _allowlistProof and _data for now (can be extended later)
    ...
    /**
     * @dev Set claim conditions
     * @param _active Whether claiming is active
     * @param _startTime When claiming can start (timestamp)
     */
